...
- [~] 5.7 **[P1]** **5.4 的檢查看不見它自己文件所標的預設做法**（見 D45，**已由 D48 取代**）。`router-dns.md` 列三種做法，第一種「DHCP 發 k8s-gateway 位址」被標為預設，**因為便宜路由器只有那一種**；而檢查問的是路由器自己解不解得出內網名稱，那種做法下路由器從不轉發，於是**永遠 FAIL、永遠扣住 dead-man ping**
  ...
  - **2026-08-16：還有第二條通往同一個結果的路，而且方向相反——永遠綠。** 由
    `jgt-appliance-b5` 在 jgt-appliance 的 LAN 上量到：`dig +short NS janncot.cc @1.1.1.1`
    回的是叢集自己 k8s-gateway 的 SOA。**它指名了 1.1.1.1，而回答它的不是 1.1.1.1。**
    該 session 因此丟棄了原本以 `dig` 取得的結果並改用 DoH 重測
    - 檢查 #18（`jg-base/.../daily-check/app/configmap.yaml:369-379`）做的是
      `dig +short @${NODE_DEFAULT_GATEWAY} internal.${SECRET_DOMAIN} A`，拿到 RFC1918 就記
      `ok`。**沒有任何一段驗證那個答案來自閘道**——只驗證了「往那個位址問，有東西回來」。
      `+short` 連 `;; SERVER:` 那行都丟掉，而即使留著也只是「我送去哪」，不是「誰回的」：
      透明攔截會偽裝來源位址
    - 攔截 53 埠正是便宜路由器常見的行為，而那正是 appliance 的出貨對象。所以 5.7 要防的
      「把真正的偵測器換成永遠綠的裝飾」**不需要有人選錯選項就會發生**——(a)/(b)/(c) 任何一個
      若實作成信任目的位址，三者共用這個洞
    - ⚠️ 這一條與 `docs/deploy/combinations.md` §6.4 的既有指示互相咬到：那裡叫人**從叢集內部**
      量（因為跨網段的 53 常被攔截），而 daily-check 本來就跑在叢集內部。兩邊都可能量到的是
      自己那一段的攔截，而不是路由器。**「從哪裡量」和「誰回答」是兩個問題，目前只處理了前者**
  - **可用的鑑別方法（未實作、未實測）**：向閘道查一個**不該存在於任何地方**的名字。沒有攔截時
    應得 NXDOMAIN；有東西回答就證明有人在替閘道作答。同型的更強版本是把查詢送到一個確定
    沒有 DNS 服務的位址（例如 TEST-NET），有回應即為攔截。兩者都便宜，且回答的是
    「誰在答」而不是「答了什麼」
  - **選 5.7 的方案時要一併決定：檢查要不要有能力知道「誰回答的」。** 只驗內容不驗來源的
    實作，三個選項都會帶著這個洞
  - **不影響 2026-08-14 那次 jg-jiahd 的判讀**：那次 `dig @10.9.9.1` 回的是 NXDOMAIN，
    而會導向 k8s-gateway 的攔截應該回出正確位址才對。所以「該路由器在 DHCP 做法下不解析
    內網名稱」這個結論站得住，沒有被這條新發現解釋掉
  - **建議在 jg-jiahd 量一次**（未做）：向閘道查一個不存在的名字，用來分辨「路由器不解析
    內網名稱」與「這個 LAN 上有東西什麼都回答」。今天這兩種情況在同一張表上會產出不同的列
  - **2026-08-17：5.7 的「要建哪一種偵測」被 ferry133 descope 為日後再議**
    （`ferry133/jg-cluster-template#6`）——DNS 設定改為交付時的員工手動動作。**但設計延後
    不等於現況可以照留。** 檢查 #18 在出貨預設（DHCP 做法）下永遠 FAIL，而 FAIL 會 ping
    `/fail`，於是每一台 appliance 每天在 healthchecks.io 上顯示 Down——**「叢集死了」與
    「有一項檢查每天紅」在那張儀表板上變成同一個狀態**，而 dead-man switch 同時罩著另外
    十七項檢查。這不是延後的改進，是會跟著出貨的缺陷
  - **決定（本 task 擁有者，2026-08-17）：分兩段，且第一段必須先落地。**
    1. **立即**：把 #18 空回應那一支從 `record fail` 降為 `record warn`。warn 不扣 dead-man
       ping，缺陷即刻停止出貨。**一行改動，在 jg-base `.../daily-check/app/configmap.yaml:374`**
    2. **目標**：改為依「宣告的做法」gating——`cluster.yaml` 宣告這座叢集用三種 router-dns
       做法的哪一種，#18 只在它看得見的做法下斷言，DHCP 做法下報「此做法無法由叢集自證」。
       這與 daily-check 既有的「未設定就印一行並 exit 0」同型（備份、memory tier 都是），
       慣例已經存在
  - ⚠️ **永久停在第一段是不可接受的終局**：一個永遠掛著的 warn 同樣會訓練所有人略過整份報告
    ——那正是 D45 一開始指出的失效。第一段是止血，不是答案；它的解除條件是第二段落地
  - **實作屬 jg-base**（檔案在那裡變動），本 repo 只記錄決定。已隨 fleet-ops 回報路由過去
  - 待決定的是探測方式，選項至少有：(a) 改問 k8s-gateway 自己（驗得到叢集側，驗不到路由器）；(b) 依實際採用的做法切換探測目標，代價是叢集要知道一件它管不到的事；(c) 由客戶端回報（與 zero-it-onboarding 1.4 同一個機制）。**先不動檢查**——選錯會把一個真實的故障偵測換成一個總是綠的裝飾
...
- [~] 8.3 **[延後：備份]** 完成還原演練：僅用備份封存 + escrow 的 `age.key`，在新叢集還原並比對資料一致
  ...
  - **2026-08-17：8.3 不隨 #6 的 descope 一起下車。** 同一次決策把 `age.key` escrow 的
    **機制**（`factory-agent` 5.5）改為員工手動動作，那個 descope 成立；但**還原演練不因為
    ...
    自動化的原因。**沒有人在等程式碼**，等的是它到底有沒有成功還原過，而答案仍是沒有
  - 兩者的差別可以一句話講完：手動化回答的是「誰來做」，而 8.3 的問題從來不是誰做，
    是**做了會不會成功**
  - 因此仍維持：**第一次對外交付之前必須跑完一次**。ferry133 已確認尚無任何一台出貨給客戶，
    所以這道閘在前方而非已被跳過；而它證的是**管線**的性質，一次即可，不是每交付一台一次
  - ⚠️ 若要連這一項也延到第一次交付之後，那應該是 ferry133 明白做的決定，**不是從 #6 的
    descope 繼承來的**。後果很具體：**第一位客戶的備份，會是這條管線第一次被測試還不還得回來**
  - ✅ **釋放條件已由 ferry133 明說（2026-08-17）**，不再是我方推定：DNS、Cloudflare 與 escrow
    三者「will be enhanced after all other process complete, **and before actually shipping
    goods to customers**」。所以 8.3 的閘門位置由他確認為**第一次出貨給客戶之前**，
    三項皆為排序而非放棄
  - 寫下這一句的理由與今天其餘各條同型：**「之後會做」與「不會做」留下的痕跡一模一樣，
    除非把條件寫下來。**條件已寫，之後不需要有人再問一次
...
- [ ] 8.3c **[延後：備份]** **保留策略從未生效，且會刪掉當日以外的全部**（2026-08-16 實測）：container 是 alpine，`apk add` 未裝 coreutils，busybox `date` 不接受 `-d "30 days ago"`，於是 `|| date -u '+%Y%m%d'` 讓 cutoff 變成今天。`BACKUP_RETAIN_DAYS` 在任何叢集上都沒有作用過
  ...
  - **2026-08-22 由 ferry133 再次裁定維持開著**：7.2 的實跑正在進行，這一項**等測試全部
    跑完再回來**。所以「備份全部延後」那條擱置在此期間**只解凍了 escrow 與 R2/物件儲存
    設定**，不含這個缺陷——7.2 的叢集會帶著它上線
  - 連帶要說清楚的：目的地若是**鏡像**複寫，這個 prune 會把刪除同步過去，兩份一起空；
    **快照**目標才回得到昨天。7.2 用的 MinIO（NAS1 → NAS2）是快照，所以損害被限制在
    「每日只留一份」，不是「全部消失」。這個區別已寫進 runbook 的 Step 5 清單
- [x] 8.4 撰寫還原程序文件，內容須與演練實際步驟逐字一致 → `docs/operations/backup-restore.md`
  - 步驟 1–4（種入基準、跑真正的 CronJob、僅憑 R2 憑證取回、密文不透明性）照 2026-08-15/16
    ...
    2026-08-16 刻意延後。未證實項目在該頁末尾列表，收斂於本 change 8.3
  - 一併帶入兩個前置：`pg_dump` 還原前必須先建 role（6.9 在 jg-jiahd 查出，否則 `OWNER TO`
    全失敗而表仍會建，看起來像成功）；還原目標 storage class 不同時，刪 PVC 前先驗叢集上的
    值（D38）
...
- [ ] 8.6 **[P1]** 每個 profile 的驗收都必須**在該 profile 上實跑到工作負載就緒**，不得只驗 `task configure` 的輸出。2c.9–2c.11 那四道渲染期缺陷全部無聲通過了 `task configure`（見 `design.md` D14）
  - **有機器了**（2026-08-22）：`e755a600-e306-1208-904b-6e3ed1880a00`（Kingchuxing 256GB
    內碟，Talos 已裝在碟上、維護模式、node unique token PERSISTENT）。ferry133 裁定
    **7.2 跑完後保留該機器供後續測試**，所以 8.6 不必再等硬體
  - 它同時是 `zero-it-onboarding` D14 出貨程序的產物，形狀就是出貨形狀——這正是 8.6 要的
    「在該 profile 上」，而不是在一台順手的機器上
  - ⚠️ 但它會帶著 8.3c 的 prune 缺陷上線（同日裁定維持開著），所以 8.6 的驗收**不能把
    「備份成功」讀成「備份可用」**：每天只會留下當日那一份
