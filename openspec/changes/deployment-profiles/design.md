...
#### 修正（2026-08-13）：不是兩條路都不通，是只有一條路且它需要一次設定

這一段原本寫成「appliance 兩條路都不通」，那個結論過重了。實測 jgt-omni 後看清楚：**split-horizon 本來就在運作**，只是需要客戶端問 `k8s-gateway`。

```
                     k8s-gateway (LAN)   公開 DNS
im.janncot.cc        10.9.1.243          2606:4700:… (Cloudflare)
internal.janncot.cc  10.9.1.241          （無記錄）
```

兩件事先前沒看清楚：

1. **WAN 名稱在 LAN 上會走捷徑。** `im.janncot.cc` 從 k8s-gateway 得到的是 `envoy-external` 的**內網**位址，所以家裡的裝置直接連內網，不繞 Cloudflare 出去再回來。這不只是省一趟——Cloudflare 或對外線路掛掉時，家裡仍然用得到。
2. **內網名稱只存在於 k8s-gateway**，而那是現行且可運作的設計，不是缺陷。

`k8s-gateway` 仍然是被動的——它不攔截流量，要有人指向它。但那是**一次安裝動作**，不是「不通」。D32 的決定就是承認並安排這一步。

`full` / `prosumer` 的 operator 本來就在做這件事（jg-jiahd 與 jcom 今天如此運作）。appliance 的差別只在於那一步由 operator 在交機時完成，而不是客戶自己。
...
split-horizon 的完整行為（見 D29 的修正）意味著 8.2 不只要驗「內網名稱可解」，還要驗「**WAN 名稱在 LAN 上解到內網位址**」。

後者是實質差異而非細節：它決定 Cloudflare 或對外線路中斷時，家裡的服務還能不能用。而它只有在路由器確實指向 `k8s-gateway` 時才成立——所以它同時是「路由器設定是否生效」的最佳探針。
...
### D47. 缺席的訊號有兩種解釋，而我們每次都挑了良性的那個

這個 change 的過程中，同一個錯誤出現了**四次**，每次換一件事、每次都沒有人當場察覺。
它不是四個 bug，是一個習慣。

| 觀察到的「什麼都沒有」 | 我們的解讀 | 實際 |
|---|---|---|
| job 印 `nothing to back up` | 這座叢集沒有資料庫 | **dump 失敗了**，而失敗訊息與正常訊息逐字相同（D46）|
| jcom 的 `/etc/cri/conf.d/hosts` 是空的 | Spegel 移除得很乾淨 | **它從來沒被安裝過**，乾淨代表「沒發生」不是「清理成功」 |
| daily-check 印 `✅ Off-site backup not configured` | 這是這座叢集的陳述，不是故障 | 對 jg-jiahd 而言它是缺口——**一個綠勾讓它看不見** |
| `gh run list` 沒有 e2e 的紀錄 | jg-base 都是直推，所以 PR workflow 不會觸發 | **它被 guard 到別人的 repo**，永遠 skipped。而且沒有 Taskfile、沒有 .taskfiles、沒有 .mise.toml——四個獨立的原因它跑不了 |

第四項最能說明問題：那個解釋**是合理的、與證據一致的、而且完全錯的**。真正的原因就在
`e2e.yaml` 第 17 行，任何人打開檔案都看得到。沒有人打開，因為已經有一個說得通的解釋了。

**通則**：一個缺席的訊號至少有兩種解釋——良性的（那件事本來就不該發生）與惡性的
（那件事該發生而失敗了，失敗的樣子恰好與沒發生一樣）。**我們每次都採用了良性的那個，
而且沒有測試另一個。**

代價是不對稱的，這才是它值得成為規則的理由：驗證良性解讀通常只要打開一個檔案、跑一個
指令；漏掉惡性解讀的代價是備份兩天沒運作、一個資料庫沒被涵蓋 92 天、一個永遠不會執行的
workflow 被當成休眠中的。

**做法**：觀察到「什麼都沒有」時，把兩種解釋都寫出來，再決定測哪一個。通常兩個都該測，
因為便宜的那個是良性解讀。

**這也是為什麼腳本要能區分它們。** D46 的修法不只是讓備份能跑——`dump_db` 現在對缺席
明講 `not installed`，而「全部失敗」與「本來就沒有資料庫」印的是不同的句子、回不同的
exit code。工具能分辨的事，人就不必記得去分辨。
...
### D49. 客戶之間的 LAN CIDR 重疊是常態，不是邊緣情況（2026-08-22）

ferry133：**大多數客戶用 `192.168.1.0/24`。** 所以「兩座叢集的 node CIDR 相同」不是需要
注意的巧合，是**預設會發生的事**——每多交付一台，重疊就多一組。

本 session 差點把它當成例外處理：測試機搬到一個同樣使用 `10.9.9.0/24` 的地點時，我把它讀成
「與 jg-jiahd 共用 LAN」並據此提出位址碰撞的疑慮。**兩者是不同地點的獨立廣播網域，只是
巧合同段。** 那個誤讀的方向很危險：它會讓人以為只要協調 CIDR 就能避開問題，而在
appliance 的出貨情境裡，**我們對客戶的網段沒有任何發言權**。

#### 後果一：跨站點的路徑一律以身分定址，不以位址

`10.9.9.157` 這個字串在 fleet 裡不唯一，而且會愈來愈不唯一。所以：

| 要做的事 | 可行 | 不可行 |
|---|---|---|
| operator 構到某台機器 | Omni 代理 + **機器 UUID**（本 session 實際用的） | `talosctl -n <LAN IP>` |
| 叢集對外 | Cloudflare tunnel（出站、具名） | 埠轉發 + 位址 |
| 機器對 Omni | SideroLink（每台一個唯一 IPv6） | LAN 位址 |

本 session 也實測到 LAN 位址那條路是走不通的：機器的 `50000`／`50001` 對 LAN refused，
Talos API 只開在 SideroLink 上。**設計已經是身分定址的，這條只是把理由寫下來。**

#### 後果二：叢集之間不能互連

KubeSpan、跨站點路由、把兩個客戶的叢集接在一起——**在重疊 CIDR 下都不成立**，而重疊是
常態。所以那些不能成為需求。jg-jiahd 開著 KubeSpan（它是多節點單一站點），appliance 不開，
現在不衝突；但那是「剛好沒事」，不是「不會有事」。

#### 後果三：識別叢集不能用網路

任何以網段、閘道位址、或 LAN 位址當作叢集識別的做法都會在第二台客戶那裡碰撞。識別要用
`cluster_name`（工單決定、決定性命名，`factory-agent` D4）與機器 UUID。
