...
**2026-08-22 重列（task 7.1）。** 原本六題，D14／D48／D29 之後有增有減，而且真正的困難
不在題數——**其中一整類會產生完全相同的觀察**：機器沒有出現在 Omni。

| 問題 | 回答機制 | 出貨形狀下的狀態 |
|---|---|---|
| 機器有沒有通電 | 觀察（燈號） | 不變 |
| 網路線有沒有插好 | 觀察（＋網路盒那端的燈） | 不變 |
| ~~開機碟對不對~~ | ~~拍照~~ | **消失**：D14 之後箱內無 USB，只有一個可開機裝置 |
| 機器有沒有從內碟開起來 | 觀察（燈號／畫面）→ 拍照 | **新**，取代上一題。失敗是韌體層級的 |
| LAN 上有沒有 DHCP、有沒有發到位址 | 探測（延後） | **新**。沒有 DHCP 或位址池耗盡，機器就沒有位址 |
| 出站 HTTPS 到 Omni 是否可達 | 探測（延後） | **新，而且是實質風險**（見下） |
| ~~UDP 是否被封鎖~~ | — | **已由設計消除**：D12 裁定一律開 gRPC tunnel |
| LAN 上位址是否被佔用 | 探測（延後） | 不變，但只在叢集建起來之後才相關 |
| 路由器 DNS 有沒有指向叢集 | 叢集自己量（D48） | **已解**：daily-check 在節點的一般解析路徑上問「答案有沒有到」 |
| ~~DNS rebinding 過濾~~ | ~~探測（延後）~~ | **消失**：擋 RFC1918 的是 Cloudflare 不是客戶路由器（D29） |

#### 出站 HTTPS：一個具體的、我們自己造成的風險

機器回連的端點是 **`https://omni-siderolink.janncot.duckdns.org`**（本次實機的 kernel
args，逐字）。`duckdns.org` 是動態 DNS 網域，而**動態 DNS 是企業防火牆與部分 ISP 會整類
封鎖的類別**——不是針對我們，是分類封鎖。

這不是客戶網路的缺陷，是**我們選的端點帶進去的風險**，而且它的失敗與「客戶還沒插電」
產生一模一樣的觀察。要降低就得換一個自有網域的端點，那是 Omni 側的設定，不在本 change
的範圍，但**必須在第一台真實交付前決定**。

#### 這張表真正的困難

上面有五題的觀察結果**完全相同**：機器沒有出現在 Omni。

```
沒插電 ／ 沒開機 ／ 沒 DHCP ／ 出站被擋 ／ 韌體沒從內碟開
          ↓
    「機器沒出現」——同一個畫面
```

所以 7.1 的價值不在列出題目，而在**承認它們共用一個症狀**：任何只回報「機器沒出現」的
工具，等於把五種原因壓成一種。這正是 CLAUDE.md 那條「什麼都沒有至少有兩種解釋」在這裡
的具體形狀，也是為什麼 §7 的探測不能只做「有沒有回應」。

延後的三題（DHCP、出站 HTTPS、LAN 位址佔用）目前只能人工升級，規格要求**明確標為延後**
而不是靜默無解。
...
#### ⚠️ 工廠那個叢集是拋棄式的，user repo 不在這裡建

D14 的工廠步驟裡也建了一個叢集，所以這條程序讀起來很容易被當成「供裝在工廠就完成了」。
**不是。** 那個叢集只為了讓 Omni 把 Talos 裝進碟，建完就刪；客戶的叢集、user repo、
tunnel、DNS 全部發生在機器落地之後。

```
工廠   ISO 開機 → 建拋棄式叢集 → 裝碟 → 刪掉 → 拔 USB → 出貨
       ↑ 這裡不建 repo

客戶   插電 → 機器用「真實網路」回連
       → factory-agent 4.3 建客戶的 Omni 叢集
       → 4.4 建 user repo          ← 在這裡
       → 4.5 tunnel / DNS
       → 4.6 從 Omni 回報的實際網路狀態推導 cluster.yaml
       → 4.7 task configure → push
```

**為什麼不能提前**：本次搬移量到同一台機器的位址由 `10.9.1.238/24` 變成
`10.9.9.157/24`。在工廠推導出來的 `node_cidr` 與 `node_default_gateway` 會是
`10.9.1.0/24`——到客戶家全錯。而 4.6 的要求逐字是「網路值一律來自 Omni 回報的機器實際
網路狀態，不接受人工輸入」，那份狀態只有機器到了客戶網路才存在。

appliance 更嚴：`lan_shared_addr` 靠 `lan-address-probe` 在**客戶的 LAN 上**以 ARP 探測
選址，工廠裡探不到。

**這個誤讀的代價是看不見的**：一份填著工廠網段的 `cluster.yaml` 會**通過 `task configure`**
（那些值在語法與 schema 上都合法），然後在客戶家整個叢集起不來——而錯誤發生在渲染前，
任何渲染後的產物都看不出問題。

**可以提前的是名字，不是內容。** `cluster_name` 由工單決定，所以 repo／tunnel／DNS 的
名稱在出貨前就算得出來，這正是 factory-agent D4「決定性命名 + 先查後建」的基礎。
內容要等機器落地。
