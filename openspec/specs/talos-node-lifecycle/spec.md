# talos-node-lifecycle Specification

## Purpose

Defines the day-two operations on a manually provisioned Talos cluster: reapplying configuration to one node, upgrading Talos per node, upgrading Kubernetes on its own, and resetting the cluster. Every operation names its target explicitly and checks its prerequisites before mutating anything, so that a mistake fails before it reaches a node rather than halfway through the fleet. Created by archiving change `revive-talos-path`.
