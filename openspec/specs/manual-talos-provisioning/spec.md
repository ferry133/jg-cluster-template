# manual-talos-provisioning Specification

## Purpose

Defines the manual Talos provisioning path: how per-node facts are declared and validated, what the rendered Talos configuration must and must not enable, and what bringing up a cluster from bare machines consists of. This is the escape hatch from the managed (Omni) path — it exists so that a cluster can be built without a management plane, and it must produce a cluster indistinguishable from a managed one once running. Created by archiving change `revive-talos-path`.
