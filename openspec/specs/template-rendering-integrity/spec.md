# template-rendering-integrity Specification

## Purpose

Defines the self-consistency properties of the rendering pipeline itself, independent of any one app: every data source a template reads is declared, no task references a variable that is defined nowhere, inherited tasks whose premise does not hold here are removed rather than left runnable, and a field never carries two conflicting defaults. These are the failures that render empty or fail late instead of loudly, so the spec also requires that they be checkable by running something, not only by reading the files. Created by archiving change `revive-talos-path`.
