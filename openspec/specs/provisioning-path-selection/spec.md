# provisioning-path-selection Specification

## Purpose

Defines how the two provisioning paths — managed (Omni) and manual Talos — coexist in a single repository: that both are supported and neither deprecated, that one path's prerequisites never block a user of the other, that path selection is declared rather than inferred, and that each path's documentation stands on its own. Created by archiving change `revive-talos-path`.
